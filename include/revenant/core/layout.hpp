...
inline bool is_control_block_valid(const ControlBlock* cb) noexcept {
    if (!cb)
        return false;
    if (cb->magic != 0x52564E54)
        return false;
    if (cb->version != 1)
        return false;
    if (cb->header_kind > 2)
        return false;
    if (cb->capacity_log2 < 12 || cb->capacity_log2 > 31)
        return false;
    if (cb->block_size_log2 < 12 || cb->block_size_log2 > 30)
        return false;
    if (cb->block_size_log2 >= cb->capacity_log2)
        return false;
    return true;
}
