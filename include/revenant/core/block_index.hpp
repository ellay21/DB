...
class BlockIndex {
  public:
    ...
    void set(std::uint64_t position) noexcept {
        ...
        std::uint32_t offset =
            static_cast<std::uint32_t>(position & ((1ULL << block_size_log2_) - 1));
    ...
    std::optional<std::uint32_t> get(std::uint32_t block_idx,
                                     std::uint32_t expected_generation) const noexcept {
        if (block_idx >= entries_.size())
            return std::nullopt;
